...
const diffContainerReorder = async ({ id, s: { sId, order }, d: { dId, newOrder } }, model) => {
  ...
  const [nullAssignees, nullComments] = await Promise.all([
    client.assignees.findMany({ where: { issueId: id } }),
    client.comments.findMany({ where: { issueId: id } }),
  ]);
  ...
  const reattatchAssignees = client.assignees.createMany({ data: nullAssignees });
  const reattatchComments = client.comments.createMany({ data: nullComments });
